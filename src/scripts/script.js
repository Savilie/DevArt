document.addEventListener('DOMContentLoaded', function() {
    ...
    function updateLinesVisibility(container, lines, mouseX, mouseY, edgeFactor = 1) {
        const radius = 300; // Увеличенный радиус видимости линий вокруг курсора
    ...
    document.addEventListener('mousemove', function(event) {
        ...
        if (mouseX >= -300 && mouseX <= containerRect.width + 300 &&
            mouseY >= -300 && mouseY <= containerRect.height + 300) {
    ...
    const highlightLayer = document.querySelector('.big-text-highlight');
